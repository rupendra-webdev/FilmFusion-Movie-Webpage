...
import logo from "../assets/FilmFusion-logo.svg";
...
const Header = () => {
  ...
  return (
    ...
    <header
      ...
      } fixed flex items-center w-full h-[60px] z-[10] translate-y-0 transition-all ${showNavBar}`}
    >
      <div className="w-full flex items-center justify-between max-w-[1200px] mx-auto px-5">
        <div className="cursor-pointer h-[50px] ">
          <img src={logo} alt="FilmFusion Logo" onClick={() => navigate("/")} />
        </div>
        ...
        <ul
          ...
          className={`${
            mobileMenu
              ? "absolute top-[60px] left-0 bg-black3 flex flex-col w-full py-5 border-t animation-mobileMenu text-white"
        ...
        >
          <li
            ...
            className={`${
              mobileMenu
                ? "text-[20px] w-full h-auto py-[15px] px-5 m-0 items-start  "
        ...
        <div className="md:hidden flex text-white text-xl gap-6 h-[60px]">
          <HiOutlineSearch
            onClick={openSearch}
            className="cursor-pointer hover:text-pink h-[60px]"
          />
          {mobileMenu ? (
            <VscChromeClose
